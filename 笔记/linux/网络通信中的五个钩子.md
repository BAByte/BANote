...
+ 防火墙：linux默认使用iptables充当防火墙，但是我们知道它只是提供了实现防火墙的基础能力而已，并不是真正意义上的防火墙软件。在一些新的linux发行版本中又出现了不基于iptables实现的防火墙。
...
虽然我们可以使用iptables实现代理和抓包，但是除了在代理服务器安装一些代理软件外，还得需要自定义Iptables的规则，我们想要和这个代理服务器建立一个加密的通信也是不行的，因为iptables并没有对数据包内容进行处理。
...
当我们创建了一个tun虚拟网卡后，系统中会出现tun0这个网络接口，可以使用ifconfig或ipconfig查看



其背后是用户态的应用程序，一般都是vpn程序，当应用程序对外发送网络数据包时，数据流向如下：

![image](https://github.com/BAByte/pic/blob/master/%E4%BC%81%E4%B8%9A%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_6fdcc698-75a4-4512-8e38-c5cbfe56b489.png?raw=true)
