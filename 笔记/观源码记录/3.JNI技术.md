...
在JNI层的异常和java，c++不一样，特别就特别在发生异常后，代码还是会继续执行，直到返回到java层才会返回异常，所以调试不方便，但是还是可以手动去截取异常的，说白了就是要小心写代码，，，，看书：p32

# 总结

这里主要就是简单的讲了JNI怎么实现java王国和c++王国的交流，我们并没有研究到native层，因为jni是c++写的，直接调用同为c++写的native层并不难！！
