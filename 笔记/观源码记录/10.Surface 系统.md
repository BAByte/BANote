...
+ GraphicBuffer是什么？我们从队列获取的这些所谓的GraphicBuffer是不是用的同一块内存呢？
+ 生产者消费者的工作机制是怎么跑的（上面其实是进行了简略的解释，但是我们并没详细分析，因为在安卓4.1后关于上层应用的缓冲机制已经换了，采用了生产者消费者的队列模式，所以我是后面才搞明白，前面就没有去讲）
+ 为什么没有看到Layer？

## GraphicBuffer

我们先看GraphicBuffer的工作机制
