...
那么，上层是如何接受这个VSync消息的呢？Google为上层设计了一个Choreographer类，翻译成中文是“编舞者”，是希望通过它来控制上层的绘制（舞蹈）节奏。既然是上层的App的draw方法，那肯定是在VIewRootImp里面，因为他管理了VIew的整个绘制过程，直接看构造函数：
...
遍历所有的事件，当有多个VSync事件到来，则只关注最近一次的事件。当拿到事件的时候就进行分发dispatchVsync（）
...
还创建了自己的一个消息队列，在哪里见过这个Choreographer呢？？在绘制那里！！！
...
这个方法我们熟悉的不能再熟悉了吧！！这里我们当时还纳闷为什么不是直接绘制，而是放在一个消息队列里面等待绘制，还搞了个同步屏障？？？而且mChoreographer的消息队列的



参考：<https://juejin.im/post/590a91ceac502e0058258293>
