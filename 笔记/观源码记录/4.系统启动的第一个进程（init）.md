...
每一个Actions或Services确定一个Section。解析init.rc文件就是为了获取里面定义的section，然后放在一个全局链表中，我们看看这些section有什么用。
...
init.rc中的service总共有三个分类：core、main和late_start（我们可以查找class_start这个command 因为这个就是用来启动服务的），下面整理了这三个分类下属的service：
...
通过fork创建一个进程，嗯就这样创建的。

那当zygote进程死了怎么办？又怎么恢复？回到init.cpp，这时候进入一个无限循环
