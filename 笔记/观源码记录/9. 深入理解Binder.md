...
每个进程在运行时系统都会把各个进程隔离，每个进程都会被分到一块内存，以便用来运行程序。在linux上，不同的进程可能会运行在不同的空间中：有的进程在内核空间，有的在用户空间。那Binder跨进程通信是如何实现的呢？就是通过Binder驱动实现的，而这个Binder驱动运行在内核空间，每个进程只需要向该驱动打开设备，并且通过mmap映射内存提供用户空间进程和内核进行Buffer数据交换，通过ioctl方式与内核进行数据交互，这样BInder驱动就作为一个中间者进行两个进程之间的传话者。
...
{
    if (mDriverFD >= 0) {
        // mmap the binder, providing a chunk of virtual address space to receive transactions.mmap操作提供了一种机制，让用户程序直接访问设备内存，这种机制，相比较在用户空间和内核空间互相拷贝数据，效率更高。主要功能：首先在内核虚拟地址空间，申请一块与用户虚拟内存相同大小的内存；然后再申请1个page大小的物理内存，再将同一块物理内存分别映射到内核虚拟地址空间和用户虚拟内存空间，从而实现了用户空间的Buffer和内核空间的Buffer同步操作的功能。
...
而主线程调用joinThreadPool（）是不传参的，我们看看不传参的结果

~~~c
joinThreadPool(bool isMain = true);
~~~

还是true嘛！！！所以我们总结一下MediaServer在进行了一系列的初始化后，他就去不断循环通信了，这不就是事件驱动型吗！！！这个MediaServer只开了两个线程去loop，这里是因为他提供4个服务，如果他只提供一个或者两个服务，那其实直接一个主线程也是可以胜任的！流程如下：
...
这里其实就是做了一层权限检查，只有pid为0或者为，AID_SYSTEM的进程才能注册，我们继续看do_add_service更加详细的代码
...
...... //接前面的代码
    ...
    if (si) {
       ...
       si->death.ptr = si;
        ...
        //这个svclist是一个list，保存了当前注册到ServiceManager中服务的信息。所以到这里已经注册完服务了！
       si->next = svclist;
   ...
   //这个方法是把需要注册的服务信息和BInder驱动进行沟通，就完成了服务的注册，这里面的代码我们后面慢慢分析，先知道代码到这里后服务已经注册完了！会将将BC_FREE_BUFFER和BC_REPLY命令协议发送给Binder驱动， 向client端发送reply.
...
我们找到IServiceManager的getService的具体实现，就是checkService（）为什么是checkService？因为上面的switch和case中getService和checkService放在了一起处理
...
原来是ProcessState提供的方法！真的是气啊，找了半天！最后会Bp转成IMediaPlayerService给IMediaDeathNotifier客户端使用！那现在又有一个问题，Bp中的Handle是哪里来的，MediaPalyerService的构造函数里面设置的吗？还是ServiceManager分配的？如果是后者，他又是怎么告诉BInder驱动MediaPlayerService的Handel的值xxx的？如果MediaPlayerService直接和Binder驱动这样交互，那还要ServiceManager干嘛？所以肯定是ServiceManager去搞的！我们看看ServiceManager的addService方法
...
transact方法我们早就分析过了，并没有处理MediaPlayerService的BpBinder
...
localBinder和remoteBinder返回些什么呢？看下面
...
{
    return NULL;
    //可以看到是返回null，你上去看看handler的值，当BpBInder为null时，Handler为0！
}

~~~

Handler的值怎么会为0呢？0不就是ServiceManager吗？但是我们现在注册的是MediaPlayerService啊!所以我们大胆的猜想:其他的服务handle的值都是动态分配的！！！但是在前面我们看ServiceManager的addService方法的时候就直接把服务的信息和handle值放在svclist里面了！！说明！在MediaPlayerService进程第一次和ServiceManager通信的时候BInder驱动就已经为他分配好了handle值了！！！所以要回到一切的开始：ServiceManager的BpBInder的第一次transact！！第一次talkWithDriver的ioctl方法，而前面的这些过程都没有看到有为MediaPlayerService生成handle，所以肯定在驱动里面搞的鬼，我们看看驱动内部对应的ioctl方法
                ...
                struct binder_thread *thread)
{
    ...
    //将用户空间bwr结构体拷贝到内核空间，这就是我们说的把用户空间的数据复制到内核空间，这里就是两个不同进程通信的关键方法
    copy_from_user(&bwr, ubuf, sizeof(bwr));
    ...

    if (bwr.write_size > 0) {
        //将数据放入目标进程，这里的目标进程就是ServiceManager！！那么这里肯定就去为MediaPlayerService生成handle的值了！！
...
//那么这里肯定就去为MediaPlayerService生成handle的值了！！
    ...
    for (; offp < off_end; offp++) {
        ...
        switch (fp->type) {
            ...
            case BINDER_TYPE_WEAK_BINDER: {
             ...
             //服务注册过程是在服务所在进程创建binder_node，在servicemanager进程创建binder_ref。 对于同一个binder_node，每个进程只会创建一个binder_ref对象。服务注册过程是在服务所在进程创建binder_node，在servicemanager进程创建binder_ref。 对于同一个binder_node，每个进程只会创建一个binder_ref对象。所以这里会先去查找，
...
现在我们知道了各个服务的handle是怎么来的了，就是动态计算出来，然后存在ServiceManager的binder_ref里面，然后BInder驱动用这个Handle值去ServiceManager的binder_ref 查出是哪个服务，然后就进行数据交换了！一切都完美了。MediaPlaService的BpBInder来源和Handle值的来源都明白了！在客户端就会被会转成IMediaPlayerService！

那么当MediaDeathNotifier使用IMediaPlayerService的接口时，MediaServer会怎么处理呢？前面已经讲过了，在IPCThreadState::executeCommand方法处理,也就是我们留下问题的地方：标记B！！注意！现在下面代码运行在的进程不是IMediaDeathNotifier，而是MediaServer进程的MediaPlayerService了！！因为你使用IMediaPlayerService接口是进行了IPC的哦！！情景要搞清楚！！
...
果不其然吧！！！其实根本不用想，都在自己的进程了，前面的BBinder肯定是MediaPlayerService是的实例。现在从Bp到BB的过程都很清楚了！！
