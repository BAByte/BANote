...
![image](https://github.com/BAByte/pic/blob/master/1615426552046.jpg?raw=true)
...
![image](https://github.com/BAByte/pic/blob/master/1615427067215.jpg?raw=true)
...
![image](https://github.com/BAByte/pic/blob/master/%E4%BC%81%E4%B8%9A%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_38277a44-7882-48ef-97d3-b01a173c9a27.png?raw=true)
...
867  ...
 866          final String apkPath = apkFile.getAbsolutePath();
867  
868          mParseError = PackageManager.INSTALL_SUCCEEDED;
869          mArchiveSourcePath = apkFile.getAbsolutePath();
870  
871          if (DEBUG_JAR) Slog.d(TAG, "Scanning base APK: " + apkPath);
872  
  					//这很重要，就是将apk加载到AssetManager示例中，所以不需要
873          final int cookie = loadApkIntoAssetManager(assets, apkPath, flags);
874  
  					//这东西
...
AssetManager：提供对应用程序的原始资产文件的访问；查看`Resources` 大多数应用程序想要检索其资源数据的方式。此类提供了一个较低级别的API，该API允许您打开和读取已与应用程序捆绑在一起的原始文件，作为简单的字节流。

所以对APK的读取其实是通过AssetManager。

导图：

![image](https://raw.githubusercontent.com/BAByte/pic/master/%E4%BC%81%E4%B8%9A%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_54503194-2fa7-4537-8e65-e97fddb27eb6.png)

接下来看初始化了xml解析器，调用了parseBaseApk的重载方法后发送了什么
...
拿到应用的Package后，看导图就是去处理版本关系，签名等校验，其实很好奇为什么由调用了一次scanPackageLI方法，这个方法长什么样做了什么？
...
注意scanPackageLI的方法签名，和我们签名看到的的scanPackageLI(解析AndroidMainifest.xml)是不一样的，我们详细看代码：
...
走到了scanPackageDirtyLI方法。好吧，这个方法1000多行，加油！
...
5610          }
   							//这里主要是设置系统的ui相关的东西，放到了mResolveInfo、mResolveActivity里面
...
5632                      mResolveActivity.processName = "system:ui";  //注意这里
...
我们知道系统界面是SystemUI。上面主要是对系统界面进行了记录，放在了一些以 mResolve开头的info实例里面，这里全局保存的原因可能是系统界面是全局使用的，mResolveActivity 网上有人说：

> 当有多个Activity响应一个Intent时，就会出现一个选择界面，这个界面就是ResolverActivity。

导图：

![image](https://github.com/BAByte/pic/blob/master/%E4%BC%81%E4%B8%9A%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_99401b9e-f3a8-4c99-b0aa-477885d4d1bb.png?raw=true)

继续往下看scanPackageDirtyLI方法

~~~java
5586      private PackageParser.Package scanPackageDirtyLI(PackageParser.Package pkg, int parseFlags,
5587              int scanFlags, long currentTime, UserHandle user) throws PackageManagerException {
  						...
5699          // Initialize package source and resource directories
              //注意到这里实例化文件对象，就是apk的资源和代码文件，
5700          File destCodeFile = new File(pkg.applicationInfo.getCodePath());
5701          File destResourceFile = new File(pkg.applicationInfo.getResourcePath());
  						...
5784              // Just create the setting, don't add it yet. For already existing packages
5785              // the PkgSetting exists already and doesn't have to be created.
                //getPackageLPw方法生成了一个pkgSetting,注意这里只是create don't add it yet. 
5786              pkgSetting = mSettings.getPackageLPw(pkg, origPackage, realName, suid, destCodeFile,
5787                      destResourceFile, pkg.applicationInfo.nativeLibraryRootDir,
5788                      pkg.applicationInfo.primaryCpuAbi,
5789                      pkg.applicationInfo.secondaryCpuAbi,
5790                      pkg.applicationInfo.flags, user, false);
  						...
}
~~~

这里出现了个PackageSetting，PackageSetting是描述应用的安装信息，给Settings使用。注释中有说只是生成，但未添加 到Settings中。所以这里只是去给应用填充PackageSetting。继续往下看scanPackageDirtyLI方法;

~~~java
5586      private PackageParser.Package scanPackageDirtyLI(PackageParser.Package pkg, int parseFlags,
5587              int scanFlags, long currentTime, UserHandle user) throws PackageManagerException {
  						...
              //给应用程序创建数据目录，也就是我们平时找的/data/data/  或者/data/system/
			        File dataPath;
5937          if (mPlatformPackage == pkg) {
5938              // The system package is special. 系统的数据目录，其实就是framework-res.apk
5939              dataPath = new File(Environment.getDataDirectory(), "system");
5940  
5941              pkg.applicationInfo.dataDir = dataPath.getPath();
5942  
5943          } else {
5944              // This is a normal package, need to make its data directory.普通应用程序和系统预置程序的数据目录
5945              dataPath = getDataPathForPackage(pkg.packageName, 0);
5946  
5947           ...
  						 if (dataPath.exists()) {
6041               ...
...
6043                          Log.v(TAG, "Want this data dir: " + dataPath);
6044                  }
6045                  //invoke installer to do the actual installation,调用安装程序进行实际安装
6046                  int ret = createDataDirsLI(pkgName, pkg.applicationInfo.uid,
6047                                             pkg.applicationInfo.seinfo);
6048                  if (ret < 0) {
6049                      // Error from installer
6050                      throw new PackageManagerException(INSTALL_FAILED_INSUFFICIENT_STORAGE,
6051                              "Unable to create data dirs [errorCode=" + ret + "]");
6052                  }
6053  
6054                  if (dataPath.exists()) {
6055                      pkg.applicationInfo.dataDir = dataPath.getPath();
6056                  } else {
6057                      Slog.w(TAG, "Unable to create data directory: " + dataPath);
6058                      pkg.applicationInfo.dataDir = null;
6059                  }
6060              }
  ...
}
...
最重要的就是createDataDirsLI方法，这里是真正对应用进行安装的地方，我们看看代码：
...
mInstaller是在 PackageManagerService构造函数传进来的，所以真正安装的软件是由Installer负责的，Installer是另一个系统服务。我打算不看这个Installer。我们先完善导图：

![image](https://github.com/BAByte/pic/blob/master/%E4%BC%81%E4%B8%9A%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_00258932-eeb3-4d7e-a6c1-c066a57c551e.png?raw=true)

接下来看看在给应用创建数据目录后，scanPackageDirtyLI方法还做了什么:
