...
到这一步就可以看出，这个线程已经做完对于消息队列应该做的事情了，那么就等Looper从消息队列中取出消息进行处理了，这里有个重要的点就是：Looper取消息的方法是在绑定这个Looper的线程中进行的，其他子线程只是操作了消息队列的存，Looper从中取出来的这个操作其实就是在他创建时的线程中执行的，这样一来我们子线程就能切换到Looper绑定的线程了。
...
## ActivityThread
这个东西的内部有一个Handler，叫H，对，就是叫 “ H ”！他的内部定义了一堆消息类型，包括四大组件的启动到停止的过程：
Activity的启动过程：
+ 做一些准备工作
+ 通过AMS告诉系统，我要做什么
+ 系统执行完后，ApplicationThread用H发送通知给主线程去调用Activity的一系列生命周期的方法
+ 我们写的代码得到执行


## 结论
从这里我们可以了解到安卓软件其实也就是两个循环在跑！！！他是消息驱动型的，所以安卓的消息机制到这里才算是完整，